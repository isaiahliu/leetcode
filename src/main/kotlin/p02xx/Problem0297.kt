...
fun main() {
    class Codec {
        ...
        fun serialize(root: TreeNode?): String {
            ...
            val vals = arrayListOf(root.`val`.toString())
            ...
            while (true) {
                ...
                repeat(nodes.size) {
                    nodes.pop()?.also {
                        vals.add(it.left?.`val`.toString())
                        vals.add(it.right?.`val`.toString())
                        nodes.add(it.left?.also { allNull = false })
                        nodes.add(it.right?.also { allNull = false })
                    }
                }
        ...
        fun deserialize(data: String): TreeNode? {
            val vals = data.trimStart('[').trimEnd(']').split(",").filter { it.isNotBlank() }
            ...
            while (index < vals.size) {
                vals[index++].toIntOrNull()?.also {
                    if (left) {
                        nodes.peek().left = TreeNode(it)
                    } else {
                        nodes.peek().right = TreeNode(it)
                    }
                }

                if (!left) {
                    nodes.pop().also {
                        it.left?.also { nodes.add(it) }
                        it.right?.also { nodes.add(it) }
                    }
                }
