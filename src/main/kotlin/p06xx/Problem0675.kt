...
fun main() {
    class Solution {
        fun cutOffTree(forest: List<List<Int>>): Int {
            val roads = hashSetOf<Pair<Int, Int>>()
            val trees = TreeMap<Int, Pair<Int, Int>>()

            forest.forEachIndexed { r, row ->
            ...
            fun Pair<Int, Int>.walkTo(target: Pair<Int, Int>): Int? {
                val visited = hashSetOf(this)
                val tasks = hashSetOf(this)
                ...
                while (tasks.isNotEmpty()) {
                    tasks.toSet().also { tasks.clear() }.forEach { (r, c) ->
                        ...
                        ).filter { it == target || it in roads && visited.add(it) }.forEach {
                            tasks.add(it)
                        }
            ...
            while (trees.isNotEmpty()) {
                val target = trees.pollFirstEntry().value

                result += pos.walkTo(target) ?: return -1
